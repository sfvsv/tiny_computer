...
class Util {
  ...
  //bool isTerminalCommandsEnabled = true
  ...
  //bool avncResizeDesktop = false 是否默认AVNC按当前屏幕大小调整分辨率
  //double avncScaleFactor = 0.0 AVNC：在当前屏幕大小的基础上调整缩放的比例。范围-1~1，对应比例4^-1~4^1
    ...
    switch (key) {
      ...
      case "autoLaunchVnc":
        return b
            ? G.prefs.getBool(key)!
            : (value) {
              G.prefs.setBool(key, value);
              return value;
            }(true);
      case "lastDate":
        return b
            ? G.prefs.getString(key)!
            : (value) {
              G.prefs.setString(key, value);
              return value;
            }("1970-01-01");
      ...
      case "termFontScale":
        return b
            ? G.prefs.getDouble(key)!
            : (value) {
              G.prefs.setDouble(key, value);
              return value;
            }(1.0);
      case "isStickyKey":
        return b
      ...
      case "avncScaleFactor":
        return b
            ? G.prefs.getDouble(key)!.clamp(-1.0, 1.0)
            : (value) {
              G.prefs.setDouble(key, value);
              return value;
            }(0.0);
      ...
      case "containersInfo":
        return G.prefs.getStringList(key)!;
    }
  }
    ...
    switch (key) {
      ...
      case "vncUrl":
        return (value) {
          ...
          "http://localhost:36082/vnc.html?host=localhost&port=36082&autoconnect=true&resize=remote&password=12345678",
        );
      case "vncUri":
        return (value) {
          addCurrentProp(key, value);
          return value;
        }("vnc://127.0.0.1:5904?VncPassword=12345678&SecurityType=2");
      case "commands":
        return (value) {
          addCurrentProp(key, value);
          return value;
  ...
  static Future<void> setCurrentProp(String key, dynamic value) async {
    await G.prefs.setStringList(
      "containersInfo",
      Util.getGlobal("containersInfo")..setAll(G.currentContainer, [
        jsonEncode(
          (jsonDecode(Util.getGlobal("containersInfo")[G.currentContainer]))
            ..update(key, (v) => value),
        ),
      ]),
    );
...
class D {
  ...
  static const String tabletDesktopSetupCommand =
      r'''mkdir -p "$HOME/.config/autostart" "$HOME/.config/environment.d" "$HOME/.local/bin" "$HOME/.config/fcitx5/conf" "$HOME/.config/xfce4/terminal"
...
EOF
cat > "$HOME/.local/bin/tiny-tablet-session-setup" <<'EOF'
#!/bin/sh
export XMODIFIERS=@im=fcitx
export GTK_IM_MODULE=fcitx
export QT_IM_MODULE=fcitx
...
if command -v fcitx5 >/dev/null 2>&1; then
  ...
  pgrep -x fcitx >/dev/null 2>&1 || fcitx -d >/dev/null 2>&1 &
fi
EOF
chmod +x "$HOME/.local/bin/tiny-tablet-session-setup"
...
EOF''';
  ...
  static const String installSimplifiedChineseInputCommand =
      r'''echo '正在安装/启用稳定简体拼音输入法（Fcitx5 Chinese Addons）...'
...
echo "稳定简体拼音输入法已安装并启用：$default_im。已移除默认英文键盘项，避免自动跳回英文。请退出并重新进入桌面。" ''';

  //默认快捷指令
  static const commands = [
    {"name": "安装稳定简体拼音", "command": installSimplifiedChineseInputCommand},
    {
      "name": "检查更新并升级",
      "command":
          "sudo dpkg --configure -a && sudo apt update && sudo apt full-upgrade -y && sudo apt autoremove -y",
    },
    ...
    {"name": "卸载Octave", "command": "sudo apt autoremove --purge -y octave"},
    {
      "name": "安装WPS",
      "command":
          r"""cat << 'EOF' | sh && sudo dpkg --configure -a && sudo apt update && sudo apt install -y /tmp/wps.deb
...
rm /tmp/wps.deb""",
    ...
    {"name": "卸载WPS", "command": "sudo apt autoremove --purge -y wps-office"},
    {
      "name": "安装CAJViewer",
      "command":
          "wget https://download.cnki.net/cajPackage/tongxinUOS/signed_cajviewer_9.5.0-25268_arm64.deb -O /tmp/caj.deb && sudo apt update && sudo apt install -y /tmp/caj.deb; rm /tmp/caj.deb",
    },
    {
      "name": "卸载CAJViewer",
      "command": "sudo apt autoremove --purge -y cajviewer",
    },
    {
      "name": "安装亿图图示",
      "command":
          "wget https://cc-download.wondershare.cc/business/prd/edrawmax_13.1.0-1_arm64_binner.deb -O /tmp/edraw.deb && sudo apt update && sudo apt install -y /tmp/edraw.deb && bash /home/tiny/.local/share/tiny/edraw/postinst; rm /tmp/edraw.deb",
    },
    {
      "name": "卸载亿图图示",
    ...
    },
    {
      "name": "安装QQ",
      "command":
          """wget \$(curl -s https://cdn-go.cn/qq-web/im.qq.com_new/latest/rainbow/linuxConfig.js | grep -oP '"armDownloadUrl":{[^}]*"deb":"\\K[^"]+') -O /tmp/qq.deb && sudo apt update && sudo apt install -y /tmp/qq.deb && sed -i 's#Exec=/opt/QQ/qq %U#Exec=/opt/QQ/qq --no-sandbox %U#g' /usr/share/applications/qq.desktop; rm /tmp/qq.deb""",
    },
    {"name": "卸载QQ", "command": "sudo apt autoremove --purge -y linuxqq"},
    {
      "name": "安装微信",
      "command":
          "wget https://dldir1v6.qq.com/weixin/Universal/Linux/WeChatLinux_arm64.deb -O /tmp/wechat.deb && sudo apt update && sudo apt install -y /tmp/wechat.deb && echo '安装完成。如果你使用微信只是为了传输文件，那么可以考虑使用支持SAF的文件管理器（如：质感文件），直接访问小小电脑所有文件。'; rm /tmp/wechat.deb",
    },
    {"name": "卸载微信", "command": "sudo apt autoremove --purge -y wechat"},
    {
      "name": "安装钉钉",
      "command":
          """wget \$(curl -sw %{redirect_url} https://www.dingtalk.com/win/d/qd=linux_arm64) -O /tmp/dingtalk.deb && sudo apt update && sudo apt install -y /tmp/dingtalk.deb libglut3.12 libglu1-mesa && sed -i 's#\\./com.alibabainc.dingtalk#\\./com.alibabainc.dingtalk --no-sandbox#g' /opt/apps/com.alibabainc.dingtalk/files/Elevator.sh; rm /tmp/dingtalk.deb""",
    },
    {
      "name": "卸载钉钉",
  ...
  static const commands4En = [
    {
      "name": "Install Stable Simplified Pinyin",
    ...
    },
    {
      ...
      "command": "sudo apt update && sudo apt install -y libreoffice",
    },
    {
      "name": "Uninstall LibreOffice",
      "command": "sudo apt autoremove --purge -y libreoffice",
    },
    {
      "name": "Install WPS",
      "command":
...
rm /tmp/wps.deb""",
    ...
    {
      "name": "Uninstall WPS",
      "command": "sudo apt autoremove --purge -y wps-office",
    },
    {
      "name": "Install EdrawMax",
      "command":
          """wget https://cc-download.wondershare.cc/business/prd/edrawmax_13.1.0-1_arm64_binner.deb -O /tmp/edraw.deb && sudo apt update && sudo apt install -y /tmp/edraw.deb && bash /home/tiny/.local/share/tiny/edraw/postinst && sudo sed -i 's/<Language V="cn"\\/>/<Language V="en"\\/>/g' /opt/apps/edrawmax/config/settings.xml; rm /tmp/edraw.deb""",
    },
    {
...
class Workflow {
  static void appendPostCommand(String command) {
    if (G.postCommand.trim().isEmpty) {
      G.postCommand = command;
      return;
    }
    G.postCommand = "${G.postCommand}\n$command";
  }

  static Future<void> ensureShortcutCommand(
    String name,
    String command, {
    bool insertFirst = true,
  }) async {
    final List commands = Util.getCurrentProp("commands") as List;
    if (commands.any((element) => element["name"] == name)) {
      return;
    }
    final updated = List.of(commands);
    final item = {"name": name, "command": command};
    if (insertFirst) {
...
""");
  ...
  static Future<void> initForFirstTime() async {
    //首先设置bootstrap
    G.updateText.value =
        AppLocalizations.of(G.homePageStateContext)!.installingBootPackage;
    await setupBootstrap();

    G.updateText.value =
        AppLocalizations.of(G.homePageStateContext)!.copyingContainerSystem;
    ...
    //-J
    G.updateText.value =
        AppLocalizations.of(G.homePageStateContext)!.installingContainerSystem;
    await Util.execute("""
export DATA_DIR=${G.dataPath}
...
"commands":${jsonEncode(Localizations.localeOf(G.homePageStateContext).languageCode == 'zh' ? D.commands : D.commands4En)}
    ...
    G.updateText.value =
        AppLocalizations.of(G.homePageStateContext)!.installationComplete;
  }
...
sed -i -E "s@^(VNC_RESOLUTION)=.*@\\1=${w}x${h}@" \$(command -v startvnc)""";
    ...
    if (Util.getGlobal("reinstallBootstrap")) {
      G.updateText.value =
          AppLocalizations.of(G.homePageStateContext)!.reinstallingBootPackage;
      await setupBootstrap();
...
clear""");
    ...
    Util.termWrite(
      (Util.getGlobal("autoLaunchVnc") as bool)
          ? ((Util.getGlobal("useX11") as bool)
              ? """mkdir -p "\$HOME/.vnc" && bash /etc/X11/xinit/Xsession &> "\$HOME/.vnc/x.log" &"""
  ...
  static Future<void> launchAvnc() async {
    await AvncFlutter.launchUsingUri(
      ...
      resizeRemoteDesktopScaleFactor:
          pow(4, Util.getGlobal("avncScaleFactor") as double).toDouble(),
    );
  }
